...
int main()
{
	...
	{
        fprintf(stderr, "nph-mjprox?%i invalid camera number %i\n", cameranumber, cameranumber);
		exit(0);
	}
	...
	if(result == -1)
	{
        fprintf(stderr, "nph-mjprox?%i failed to connect to port %i: %s\n", cameranumber, PORTBASE + cameranumber, strerror(errno));
		exit(1);
	}
	...
	for(;;)
	{
		...
		if (n == -1)
		{
			/* read was interrupted? then just do it again */
			if (errno == EINTR)
				continue;
            fprintf(stderr, "nph-mjprox?%i failed to read from buffer: %s\n", cameranumber, strerror(errno));
            close(sockfd);
			exit(1);
		}
		...
		while(n > 0)
		{
			...
			if (nsend == -1)
			{
				if (errno == EINTR)
					continue;
                fprintf(stderr, "nph-mjprox?%i failed to write to buffer: %s\n", cameranumber, strerror(errno));
	            close(sockfd);
				exit(1);
			}
			else if (nsend == 0)
            {
                ...
                fprintf(stderr, "nph-mjprox?%i failed to write to buffer.  Wierd issue with stdout being closed?: %s\n", cameranumber, strerror(errno));
	            close(sockfd);
				exit(1);
			}
